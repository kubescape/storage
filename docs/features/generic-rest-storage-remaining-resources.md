...
Each is gated behind its own `config.Config.Custom<Resource>RestEnabled` flag (default `true` as
of 2026-08-31, after live validation against armo-dev-stage found zero behavioral divergence
across all 11 Phase 4 resources); the old `genericregistry.Store`-based implementation remains
available as a fallback (set the flag to `false`) and as the differential-testing reference for
every one of them. The three "virtual CRD" resources
