...
func TestStorageImplWatchReturnsDifferentWatchers(t *testing.T) {
	type args struct {
		ctx  context.Context
		key  string
...
func TestStorageImplPublishesToMatchingWatch(t *testing.T) {
	tt := []struct {
		name                 string
		inputKey             string
		inputObject          *v1beta1.SBOMSPDXv2p3
		parallelWatchesCount int
		expectedEvent        watch.Event
	}{
		{
			name:     "Create should publish to the appropriate single channel",
			inputKey: "/spdx.softwarecomposition.kubescape.io/sbomspdxv2p3s/kubescape",
			inputObject: &v1beta1.SBOMSPDXv2p3{
				ObjectMeta: v1.ObjectMeta{
					Name: "some-sbom",
				},
			},
			parallelWatchesCount: 1,
			expectedEvent: watch.Event{
				Type: watch.Added,
		...
		{
			name:     "Create should publish to the appropriate multiple channels",
			inputKey: "/spdx.softwarecomposition.kubescape.io/sbomspdxv2p3s/kubescape",
			inputObject: &v1beta1.SBOMSPDXv2p3{
				ObjectMeta: v1.ObjectMeta{
					Name: "some-sbom",
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 0; i < tc.parallelWatchesCount; i++ {
				w, _ := s.Watch(ctx, tc.inputKey, opts)
				watches = append(watches, w)
			}

			var ttl uint64 = 0
			var out runtime.Object
			s.Create(ctx, tc.inputKey, tc.inputObject, out, ttl)

			for idx := range watches {
				w := watches[idx]
				gotEvent := <-w.ResultChan()
