...
func (a *ContainerProfileProcessor) AfterCreate(ctx context.Context, conn *sqlite.Conn, object runtime.Object) error {
	...
	logger.L().Debug("new TS container profile received", loggerhelpers.Interface("profile", profile))
	...
	if err != nil {
		logger.L().Ctx(ctx).Error("failed to write time series data for container profile",
	...
	logger.L().Info("TS container profile registered",
		loggerhelpers.String("name", profile.Name),
		loggerhelpers.String("namespace", namespace),
		loggerhelpers.String("completion", completion),
		loggerhelpers.String("seriesID", seriesID),
...
func (a *ContainerProfileProcessor) PreSave(ctx context.Context, conn *sqlite.Conn, object runtime.Object) error {
	...
	if err == nil {
		...
		logger.L().Debug("failed to get sbom name", loggerhelpers.Error(err), loggerhelpers.String("imageTag", profile.Spec.ImageTag), loggerhelpers.String("imageID", profile.Spec.ImageID))
	}
...
func (a *ContainerProfileProcessor) SetStorage(storageImpl *StorageImpl) {
	...
	if a.interval > 0 {
		go a.startConsolidationTask()
	}
}

func (a *ContainerProfileProcessor) startConsolidationTask() {
	for {
		logger.L().Info("started consolidation task", loggerhelpers.String("interval", a.interval.String()))
		err := a.consolidateTimeSeries()
		if err != nil {
			logger.L().Error("failed to complete consolidation task", loggerhelpers.Error(err))
		} else {
			logger.L().Info("consolidation task completed successfully")
		}
...
func (a *ContainerProfileProcessor) consolidateTimeSeries() error {
	...
	for _, key := range keys {
		logger.L().Info("consolidating data for key", loggerhelpers.String("key", key))
...
func (a *ContainerProfileProcessor) updateProfile(ctx context.Context, conn *sqlite.Conn, timeSeries map[string][]TimeSeriesContainers, key string, profile softwarecomposition.ContainerProfile, prefix string, root string, namespace string) ([]string, error) {
	...
	for seriesID := range timeSeries {
		...
		for k, ts := range timeSeries[seriesID] {
			if ts.HasData {
				...
				logger.L().Info("loading TS profile", loggerhelpers.String("key", tsKey))
	...
	if !newData {
		logger.L().Info("no new data, skip saving profile", loggerhelpers.String("key", key))
		return processed, nil
	}
	// verify we have a valid profile before writing it to disk
	if _, ok := profile.Annotations[helpers.InstanceIDMetadataKey]; !ok {
		logger.L().Info("skip saving invalid profile", loggerhelpers.String("key", key), loggerhelpers.Interface("profile", profile))
		return processed, nil
	}
	...
	tryUpdateApplicationProfile := func(input runtime.Object, res storage.ResponseMeta) (runtime.Object, *uint64, error) {
		...
		ap.Annotations = map[string]string{
			...
			helpers.WlidMetadataKey:       profile.Annotations[helpers.WlidMetadataKey],
		}
		ap.Labels = profile.Labels
