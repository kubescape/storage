...
// WorkloadConfigurationScanSummary is namespaced, but its Strategy is a no-op --
...
// mutable field used to prove Update/KeyDerivation actually took effect is
// Spec.Severities.Critical (an int64 leaf reachable without indexing into
// any slice, unlike seccompprofiles' Spec.Containers).
...
// (checklist item 1).
func normalize(t *testing.T, obj runtime.Object) *softwarecomposition.WorkloadConfigurationScanSummary {
	t.Helper()
	vms, ok := obj.(*softwarecomposition.WorkloadConfigurationScanSummary)
	require.True(t, ok, "expected *softwarecomposition.WorkloadConfigurationScanSummary, got %T", obj)
	require.NotEmpty(t, vms.UID, "UID must be set")
	require.False(t, vms.CreationTimestamp.IsZero(), "creationTimestamp must be set")
	out := vms.DeepCopy()
	out.UID = ""
...
func workloadConfigScanSummary(name string, criticalAll int64) *softwarecomposition.WorkloadConfigurationScanSummary {
	return &softwarecomposition.WorkloadConfigurationScanSummary{
		ObjectMeta: metav1.ObjectMeta{Name: name},
		Spec:       softwarecomposition.WorkloadConfigurationScanSummarySpec{Severities: softwarecomposition.WorkloadConfigurationScanSeveritiesSummary{Critical: criticalAll}},
	}
}

func mustCreate(t *testing.T, ctx context.Context, r rest.StandardStorage, obj *softwarecomposition.WorkloadConfigurationScanSummary) runtime.Object {
	t.Helper()
...
func TestDifferential_CreateExplicitName(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	vms := workloadConfigScanSummary("vms-a", 1)

	oldOut := mustCreate(t, ctx, h.oldREST, vms)
	newOut := mustCreate(t, ctx, h.newREST, vms)
	...
	_, err = h.oldREST.Create(ctx, vms.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	assert.True(t, apierrors.IsAlreadyExists(err), "old: %v", err)
	_, err = h.newREST.Create(ctx, vms.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
...
func TestDifferential_CreateGenerateName(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	vms := &softwarecomposition.WorkloadConfigurationScanSummary{
		ObjectMeta: metav1.ObjectMeta{GenerateName: "vms-"},
		Spec:       softwarecomposition.WorkloadConfigurationScanSummarySpec{Severities: softwarecomposition.WorkloadConfigurationScanSeveritiesSummary{Critical: 1}},
	}
	...
	assert.True(t, strings.HasPrefix(oldMeta.GetName(), "vms-"), "old name: %s", oldMeta.GetName())
	assert.True(t, strings.HasPrefix(newMeta.GetName(), "vms-"), "new name: %s", newMeta.GetName())
	...
	oldOut2 := mustCreate(t, ctx, h.oldREST, vms)
	newOut2 := mustCreate(t, ctx, h.newREST, vms)
...
func TestDifferential_CreateSucceeds(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	vms := workloadConfigScanSummary("vms-with-severities", 3)

	oldOut, err := h.oldREST.Create(ctx, vms.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	require.NoError(t, err)
	newOut, err := h.newREST.Create(ctx, vms.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	require.NoError(t, err)
	...
	oldFresh, err := h.oldREST.Get(ctx, "vms-with-severities", &metav1.GetOptions{})
	require.NoError(t, err)
	newFresh, err := h.newREST.Get(ctx, "vms-with-severities", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, int64(3), oldFresh.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)
	assert.Equal(t, int64(3), newFresh.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)
}
...
func TestDifferential_NamespaceIsolation(t *testing.T) {
	...
	mustCreate(t, ctxA, h.oldREST, workloadConfigScanSummary("shared-name", 1))
	mustCreate(t, ctxB, h.oldREST, workloadConfigScanSummary("shared-name", 2))
	mustCreate(t, ctxA, h.newREST, workloadConfigScanSummary("shared-name", 1))
	mustCreate(t, ctxB, h.newREST, workloadConfigScanSummary("shared-name", 2))

	oldA, err := h.oldREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	oldB, err := h.oldREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, int64(1), oldA.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)
	assert.Equal(t, int64(2), oldB.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)

	newA, err := h.newREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	newB, err := h.newREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, int64(1), newA.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)
	assert.Equal(t, int64(2), newB.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)
...
func TestDifferential_KeyDerivationMatches(t *testing.T) {
	...
	mustCreate(t, ctx, h.oldREST, workloadConfigScanSummary("via-old", 1))
	...
	assert.Equal(t, int64(1), viaNew.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)

	// Written via NEW, read via OLD.
	mustCreate(t, ctx, h.newREST, workloadConfigScanSummary("via-new", 2))
	...
	assert.Equal(t, int64(2), viaOld.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)
	...
	updated.Spec.Severities.Critical = 9
	...
	require.NoError(t, err)
	assert.Equal(t, int64(9), viaNewAfterUpdate.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)
}
...
func TestDifferential_UpdateResourceVersion(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := workloadConfigScanSummary("srv", 1)

	oldCreated := mustCreate(t, ctx, h.oldREST, base).(*softwarecomposition.WorkloadConfigurationScanSummary)
	newCreated := mustCreate(t, ctx, h.newREST, base).(*softwarecomposition.WorkloadConfigurationScanSummary)

	// Correct resourceVersion succeeds.
	updatedOld := oldCreated.DeepCopy()
	updatedOld.Spec.Severities.Critical = 5
	...
	updatedNew.Spec.Severities.Critical = 5
...
func TestDifferential_UpdateCarriesOverProtectedFields(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base2 := workloadConfigScanSummary("srv2", 1)
	...
	require.NoError(t, err)
	oldCreated2 := oldFetched.(*softwarecomposition.WorkloadConfigurationScanSummary)
	newCreated2 := newFetched.(*softwarecomposition.WorkloadConfigurationScanSummary)
...
func TestDifferential_DeleteWithFinalizer(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := &softwarecomposition.WorkloadConfigurationScanSummary{
	...
	clearedOld := oldFresh.(*softwarecomposition.WorkloadConfigurationScanSummary).DeepCopy()
	...
	clearedNew := newFresh.(*softwarecomposition.WorkloadConfigurationScanSummary).DeepCopy()
...
func TestDifferential_DeleteCollection(t *testing.T) {
	...
	for _, name := range names {
		mustCreate(t, ctx, h.oldREST, workloadConfigScanSummary(name, 1))
		mustCreate(t, ctx, h.newREST, workloadConfigScanSummary(name, 1))
	}
...
func TestDifferential_DryRunCreate(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	vms := workloadConfigScanSummary("srv", 1)

	var oldOut runtime.Object
	var err error
	assert.NotPanics(t, func() {
		oldOut, err = h.oldREST.Create(ctx, vms.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{DryRun: []string{metav1.DryRunAll}})
	...
	newOut, err := h.newREST.Create(ctx, vms.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{DryRun: []string{metav1.DryRunAll}})
	require.NoError(t, err)
	newMeta, err := meta.Accessor(newOut)
	require.NoError(t, err)
...
func TestDifferential_DryRunUpdate(t *testing.T) {
	...
	updatedOld := oldCreated.DeepCopy()
	updatedOld.Spec.Severities.Critical = 7
	var oldOut runtime.Object
	var err error
	assert.NotPanics(t, func() {
	...
	require.NoError(t, err)
	assert.Equal(t, int64(7), oldOut.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)

	updatedNew := newCreated.DeepCopy()
	updatedNew.Spec.Severities.Critical = 7
	...
	require.NoError(t, err)
	assert.Equal(t, int64(7), newOut.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical)

	// Neither implementation actually persisted the change.
	persistedOld, err := h.oldREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, int64(1), persistedOld.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical, "old: dry-run update must not persist")

	persistedNew, err := h.newREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, int64(1), persistedNew.(*softwarecomposition.WorkloadConfigurationScanSummary).Spec.Severities.Critical, "new: dry-run update must not persist")
}
...
func TestDifferential_ProtobufRoundTrip(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			versioned, ok := versionedOut.(*v1beta1.WorkloadConfigurationScanSummary)
			require.True(t, ok, "expected *v1beta1.WorkloadConfigurationScanSummary, got %T", versionedOut)

			data, err := versioned.Marshal()
			require.NoError(t, err)
			require.NotEmpty(t, data)

			var decoded v1beta1.WorkloadConfigurationScanSummary
