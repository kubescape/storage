...
// against both, asserting identical externally-observable results. Modeled
// directly on
// pkg/registry/softwarecomposition/openvulnerabilityexchange/custom_rest_test.go:
// WorkloadConfigurationScan is namespaced, but its Strategy is a no-op --
// Validate/ValidateUpdate always return an empty field.ErrorList
// (strategy.go) -- so this suite has no validation-rejection cases. The
// mutable field used to prove Update/KeyDerivation took effect is
// Spec.RelatedObjects (a slice), so -- like seccompprofiles -- any mutation
// starting from a value returned directly by Create/Update (which only
// carries ObjectMeta/SchemaVersion, see saveObject/extractFields in
// storage.go) assigns a whole new slice rather than indexing into the
// empty one Create/Update actually returned.
...
// namespace+name: a write through one and a read through the other must
// see the same object.
...
// (checklist item 1).
func normalize(t *testing.T, obj runtime.Object) *softwarecomposition.WorkloadConfigurationScan {
	t.Helper()
	wcs, ok := obj.(*softwarecomposition.WorkloadConfigurationScan)
	require.True(t, ok, "expected *softwarecomposition.WorkloadConfigurationScan, got %T", obj)
	require.NotEmpty(t, wcs.UID, "UID must be set")
	require.False(t, wcs.CreationTimestamp.IsZero(), "creationTimestamp must be set")
	out := wcs.DeepCopy()
	out.UID = ""
...
func workloadConfigurationScan(name, relatedObjName string) *softwarecomposition.WorkloadConfigurationScan {
	return &softwarecomposition.WorkloadConfigurationScan{
		ObjectMeta: metav1.ObjectMeta{Name: name},
		Spec:       softwarecomposition.WorkloadConfigurationScanSpec{RelatedObjects: []softwarecomposition.WorkloadScanRelatedObject{{Name: relatedObjName}}},
	}
}

func mustCreate(t *testing.T, ctx context.Context, r rest.StandardStorage, obj *softwarecomposition.WorkloadConfigurationScan) runtime.Object {
	t.Helper()
...
func TestDifferential_CreateExplicitName(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	wcs := workloadConfigurationScan("wcs-a", "obj1")

	oldOut := mustCreate(t, ctx, h.oldREST, wcs)
	newOut := mustCreate(t, ctx, h.newREST, wcs)
	...
	_, err = h.oldREST.Create(ctx, wcs.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	assert.True(t, apierrors.IsAlreadyExists(err), "old: %v", err)
	_, err = h.newREST.Create(ctx, wcs.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
...
func TestDifferential_CreateGenerateName(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	wcs := &softwarecomposition.WorkloadConfigurationScan{
		ObjectMeta: metav1.ObjectMeta{GenerateName: "wcs-"},
		Spec:       softwarecomposition.WorkloadConfigurationScanSpec{RelatedObjects: []softwarecomposition.WorkloadScanRelatedObject{{Name: "obj1"}}},
	}
	...
	assert.True(t, strings.HasPrefix(oldMeta.GetName(), "wcs-"), "old name: %s", oldMeta.GetName())
	assert.True(t, strings.HasPrefix(newMeta.GetName(), "wcs-"), "new name: %s", newMeta.GetName())
	...
	oldOut2 := mustCreate(t, ctx, h.oldREST, wcs)
	newOut2 := mustCreate(t, ctx, h.newREST, wcs)
...
// openvulnerabilityexchange for that pattern).
func TestDifferential_CreateSucceeds(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	wcs := workloadConfigurationScan("wcs-with-related", "obj1")

	oldOut, err := h.oldREST.Create(ctx, wcs.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	require.NoError(t, err)
	newOut, err := h.newREST.Create(ctx, wcs.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	require.NoError(t, err)
	...
	oldFresh, err := h.oldREST.Get(ctx, "wcs-with-related", &metav1.GetOptions{})
	require.NoError(t, err)
	newFresh, err := h.newREST.Get(ctx, "wcs-with-related", &metav1.GetOptions{})
	require.NoError(t, err)
	require.Len(t, oldFresh.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects, 1)
	require.Len(t, newFresh.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects, 1)
	assert.Equal(t, "obj1", oldFresh.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)
	assert.Equal(t, "obj1", newFresh.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)
}

// --- Namespace scoping ---

// TestDifferential_NamespaceIsolation proves objects with the same name in
// different namespaces don't collide, identically for both implementations.
func TestDifferential_NamespaceIsolation(t *testing.T) {
	h := newHarness(t)
	ctxA := testContext("ns-a")
	ctxB := testContext("ns-b")

	mustCreate(t, ctxA, h.oldREST, workloadConfigurationScan("shared-name", "obj-a"))
	mustCreate(t, ctxB, h.oldREST, workloadConfigurationScan("shared-name", "obj-b"))
	mustCreate(t, ctxA, h.newREST, workloadConfigurationScan("shared-name", "obj-a"))
	mustCreate(t, ctxB, h.newREST, workloadConfigurationScan("shared-name", "obj-b"))

	oldA, err := h.oldREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	oldB, err := h.oldREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "obj-a", oldA.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)
	assert.Equal(t, "obj-b", oldB.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)

	newA, err := h.newREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	newB, err := h.newREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "obj-a", newA.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)
	assert.Equal(t, "obj-b", newB.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)

	// A List scoped to ns-a must not see ns-b's object, for either implementation.
...
// for the same namespace+name: a Create through the OLD implementation must
// be readable through the NEW implementation and vice versa.
func TestDifferential_KeyDerivationMatches(t *testing.T) {
	...
	mustCreate(t, ctx, h.oldREST, workloadConfigurationScan("via-old", "obj1"))
	...
	assert.Equal(t, "obj1", viaNew.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)

	// Written via NEW, read via OLD.
	mustCreate(t, ctx, h.newREST, workloadConfigurationScan("via-new", "obj2"))
	...
	assert.Equal(t, "obj2", viaOld.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)

	// Update through the OLD implementation of an object created via OLD,
	// read the update via NEW.
	created, err := h.oldREST.Get(ctx, "via-old", &metav1.GetOptions{})
	require.NoError(t, err)
	updated := created.(*softwarecomposition.WorkloadConfigurationScan).DeepCopy()
	updated.Spec.RelatedObjects[0].Name = "obj1-updated"
	_, _, err = h.oldREST.Update(ctx, "via-old", rest.DefaultUpdatedObjectInfo(updated), rest.ValidateAllObjectFunc, rest.ValidateAllObjectUpdateFunc, false, &metav1.UpdateOptions{})
	require.NoError(t, err)
	viaNewAfterUpdate, err := h.newREST.Get(ctx, "via-old", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "obj1-updated", viaNewAfterUpdate.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)
}
...
func TestDifferential_UpdateResourceVersion(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := workloadConfigurationScan("srv", "obj1")

	oldCreated := mustCreate(t, ctx, h.oldREST, base).(*softwarecomposition.WorkloadConfigurationScan)
	newCreated := mustCreate(t, ctx, h.newREST, base).(*softwarecomposition.WorkloadConfigurationScan)

	// Correct resourceVersion succeeds. oldCreated/newCreated come straight
	// from mustCreate's return value, which only carries
	// ObjectMeta/SchemaVersion -- Spec.RelatedObjects is empty there, so the
	// update assigns a whole new slice rather than indexing into an empty one.
	updatedOld := oldCreated.DeepCopy()
	updatedOld.Spec.RelatedObjects = []softwarecomposition.WorkloadScanRelatedObject{{Name: "updated"}}
	...
	updatedNew.Spec.RelatedObjects = []softwarecomposition.WorkloadScanRelatedObject{{Name: "updated"}}
...
func TestDifferential_UpdateCarriesOverProtectedFields(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base2 := workloadConfigurationScan("srv2", "obj1")
	mustCreate(t, ctx, h.oldREST, base2)
	mustCreate(t, ctx, h.newREST, base2)

	oldFetched, err := h.oldREST.Get(ctx, "srv2", &metav1.GetOptions{})
	require.NoError(t, err)
	newFetched, err := h.newREST.Get(ctx, "srv2", &metav1.GetOptions{})
	require.NoError(t, err)
	oldCreated2 := oldFetched.(*softwarecomposition.WorkloadConfigurationScan)
	newCreated2 := newFetched.(*softwarecomposition.WorkloadConfigurationScan)
...
func TestDifferential_DeleteWithFinalizer(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := &softwarecomposition.WorkloadConfigurationScan{
	...
	clearedOld := oldFresh.(*softwarecomposition.WorkloadConfigurationScan).DeepCopy()
	...
	clearedNew := newFresh.(*softwarecomposition.WorkloadConfigurationScan).DeepCopy()
...
func TestDifferential_DeleteCollection(t *testing.T) {
	...
	for _, name := range names {
		mustCreate(t, ctx, h.oldREST, workloadConfigurationScan(name, "obj1"))
		mustCreate(t, ctx, h.newREST, workloadConfigurationScan(name, "obj1"))
	}
...
// limitation rather than knownservers' real-event-delivery equivalent:
// StorageImpl.Watch (storage.go) explicitly rejects any key that parses to
// a non-empty namespace segment (K8sPathToKeys) and answers with an idle
// (open, event-free) watch instead -- see the "FIXME find an alternative to
// fix NS deletion" comment there. genericrest.Store's Watch constructs its
// key exactly the way genericregistry.Store's WatchPredicate does (a
// namespace-scoped KeyRootFunc), so both implementations hit this same
...
func TestDifferential_Watch(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err = r.Create(ctx, workloadConfigurationScan("srv", "obj1"), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
			require.NoError(t, err)
			...
			case ev, ok := <-w.ResultChan():
				...
				created, ok := ev.Object.(*softwarecomposition.WorkloadConfigurationScan)
				require.True(t, ok, "expected *WorkloadConfigurationScan, got %T", ev.Object)
...
func TestDifferential_DryRunCreate(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	wcs := workloadConfigurationScan("srv", "obj1")

	var oldOut runtime.Object
	var err error
	assert.NotPanics(t, func() {
		oldOut, err = h.oldREST.Create(ctx, wcs.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{DryRun: []string{metav1.DryRunAll}})
	...
	newOut, err := h.newREST.Create(ctx, wcs.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{DryRun: []string{metav1.DryRunAll}})
	require.NoError(t, err)
	newMeta, err := meta.Accessor(newOut)
	require.NoError(t, err)
...
func TestDifferential_DryRunUpdate(t *testing.T) {
	...
	updatedOld := oldCreated.DeepCopy()
	updatedOld.Spec.RelatedObjects = []softwarecomposition.WorkloadScanRelatedObject{{Name: "changed"}}
	var oldOut runtime.Object
	var err error
	assert.NotPanics(t, func() {
	...
	require.NoError(t, err)
	assert.Equal(t, "changed", oldOut.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)

	updatedNew := newCreated.DeepCopy()
	updatedNew.Spec.RelatedObjects = []softwarecomposition.WorkloadScanRelatedObject{{Name: "changed"}}
	...
	require.NoError(t, err)
	assert.Equal(t, "changed", newOut.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name)

	// Neither implementation actually persisted the change.
	persistedOld, err := h.oldREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "obj1", persistedOld.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name, "old: dry-run update must not persist")

	persistedNew, err := h.newREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "obj1", persistedNew.(*softwarecomposition.WorkloadConfigurationScan).Spec.RelatedObjects[0].Name, "new: dry-run update must not persist")
}
...
func TestDifferential_ProtobufRoundTrip(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			versioned, ok := versionedOut.(*v1beta1.WorkloadConfigurationScan)
			require.True(t, ok, "expected *v1beta1.WorkloadConfigurationScan, got %T", versionedOut)

			data, err := versioned.Marshal()
			require.NoError(t, err)
			require.NotEmpty(t, data)

			var decoded v1beta1.WorkloadConfigurationScan
