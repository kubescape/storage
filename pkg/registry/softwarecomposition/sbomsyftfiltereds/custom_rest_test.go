...
// SBOMSyftFiltered is namespaced, but (unlike OpenVulnerabilityExchangeContainer)
// its Strategy is a no-op -- Validate/ValidateUpdate always return an empty
// field.ErrorList (strategy.go) -- so this suite has no validation-rejection
// cases.
...
// (checklist item 1).
func normalize(t *testing.T, obj runtime.Object) *softwarecomposition.SBOMSyftFiltered {
	t.Helper()
	sf, ok := obj.(*softwarecomposition.SBOMSyftFiltered)
	require.True(t, ok, "expected *softwarecomposition.SBOMSyftFiltered, got %T", obj)
	require.NotEmpty(t, sf.UID, "UID must be set")
	require.False(t, sf.CreationTimestamp.IsZero(), "creationTimestamp must be set")
	out := sf.DeepCopy()
	out.UID = ""
...
func sbomSyftFiltered(name, toolName string) *softwarecomposition.SBOMSyftFiltered {
	return &softwarecomposition.SBOMSyftFiltered{
		ObjectMeta: metav1.ObjectMeta{Name: name},
		Spec:       softwarecomposition.SBOMSyftSpec{Metadata: softwarecomposition.SPDXMeta{Tool: softwarecomposition.ToolMeta{Name: toolName}}},
	}
}

func mustCreate(t *testing.T, ctx context.Context, r rest.StandardStorage, obj *softwarecomposition.SBOMSyftFiltered) runtime.Object {
	t.Helper()
...
func TestDifferential_CreateExplicitName(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sf := sbomSyftFiltered("sf-a", "syft1")

	oldOut := mustCreate(t, ctx, h.oldREST, sf)
	newOut := mustCreate(t, ctx, h.newREST, sf)
	...
	_, err = h.oldREST.Create(ctx, sf.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	assert.True(t, apierrors.IsAlreadyExists(err), "old: %v", err)
	_, err = h.newREST.Create(ctx, sf.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
...
func TestDifferential_CreateGenerateName(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sf := &softwarecomposition.SBOMSyftFiltered{
		ObjectMeta: metav1.ObjectMeta{GenerateName: "sf-"},
		Spec:       softwarecomposition.SBOMSyftSpec{Metadata: softwarecomposition.SPDXMeta{Tool: softwarecomposition.ToolMeta{Name: "syft1"}}},
	}
	...
	assert.True(t, strings.HasPrefix(oldMeta.GetName(), "sf-"), "old name: %s", oldMeta.GetName())
	assert.True(t, strings.HasPrefix(newMeta.GetName(), "sf-"), "new name: %s", newMeta.GetName())
	...
	oldOut2 := mustCreate(t, ctx, h.oldREST, sf)
	newOut2 := mustCreate(t, ctx, h.newREST, sf)
...
func TestDifferential_CreateSucceeds(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sf := sbomSyftFiltered("sf-with-tool", "syft1")

	oldOut, err := h.oldREST.Create(ctx, sf.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	require.NoError(t, err)
	newOut, err := h.newREST.Create(ctx, sf.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	require.NoError(t, err)
	...
	oldFresh, err := h.oldREST.Get(ctx, "sf-with-tool", &metav1.GetOptions{})
	require.NoError(t, err)
	newFresh, err := h.newREST.Get(ctx, "sf-with-tool", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "syft1", oldFresh.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)
	assert.Equal(t, "syft1", newFresh.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)
}
...
func TestDifferential_NamespaceIsolation(t *testing.T) {
	...
	mustCreate(t, ctxA, h.oldREST, sbomSyftFiltered("shared-name", "tool-a"))
	mustCreate(t, ctxB, h.oldREST, sbomSyftFiltered("shared-name", "tool-b"))
	mustCreate(t, ctxA, h.newREST, sbomSyftFiltered("shared-name", "tool-a"))
	mustCreate(t, ctxB, h.newREST, sbomSyftFiltered("shared-name", "tool-b"))

	oldA, err := h.oldREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	oldB, err := h.oldREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "tool-a", oldA.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)
	assert.Equal(t, "tool-b", oldB.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)

	newA, err := h.newREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	newB, err := h.newREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "tool-a", newA.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)
	assert.Equal(t, "tool-b", newB.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)
...
func TestDifferential_KeyDerivationMatches(t *testing.T) {
	...
	mustCreate(t, ctx, h.oldREST, sbomSyftFiltered("via-old", "tool1"))
	...
	assert.Equal(t, "tool1", viaNew.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)

	// Written via NEW, read via OLD.
	mustCreate(t, ctx, h.newREST, sbomSyftFiltered("via-new", "tool2"))
	...
	assert.Equal(t, "tool2", viaOld.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)
	...
	require.NoError(t, err)
	updated := created.(*softwarecomposition.SBOMSyftFiltered).DeepCopy()
	updated.Spec.Metadata.Tool.Name = "tool1-updated"
	...
	require.NoError(t, err)
	assert.Equal(t, "tool1-updated", viaNewAfterUpdate.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)
}
...
func TestDifferential_UpdateResourceVersion(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := sbomSyftFiltered("srv", "tool1")

	oldCreated := mustCreate(t, ctx, h.oldREST, base).(*softwarecomposition.SBOMSyftFiltered)
	newCreated := mustCreate(t, ctx, h.newREST, base).(*softwarecomposition.SBOMSyftFiltered)

	// Correct resourceVersion succeeds.
	updatedOld := oldCreated.DeepCopy()
	updatedOld.Spec.Metadata.Tool.Name = "updated"
	...
	updatedNew.Spec.Metadata.Tool.Name = "updated"
...
func TestDifferential_UpdateCarriesOverProtectedFields(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base2 := sbomSyftFiltered("srv2", "tool1")
	...
	require.NoError(t, err)
	oldCreated2 := oldFetched.(*softwarecomposition.SBOMSyftFiltered)
	newCreated2 := newFetched.(*softwarecomposition.SBOMSyftFiltered)
...
func TestDifferential_DeleteWithFinalizer(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := &softwarecomposition.SBOMSyftFiltered{
		ObjectMeta: metav1.ObjectMeta{Name: "srv", Finalizers: []string{"test/finalizer"}},
		Spec:       softwarecomposition.SBOMSyftSpec{Metadata: softwarecomposition.SPDXMeta{Tool: softwarecomposition.ToolMeta{Name: "tool1"}}},
	}
	mustCreate(t, ctx, h.oldREST, base)
	mustCreate(t, ctx, h.newREST, base)
	...
	clearedOld := oldFresh.(*softwarecomposition.SBOMSyftFiltered).DeepCopy()
	...
	clearedNew := newFresh.(*softwarecomposition.SBOMSyftFiltered).DeepCopy()
...
func TestDifferential_DeleteCollection(t *testing.T) {
	...
	for _, name := range names {
		mustCreate(t, ctx, h.oldREST, sbomSyftFiltered(name, "tool1"))
		mustCreate(t, ctx, h.newREST, sbomSyftFiltered(name, "tool1"))
	}
...
func TestDifferential_Watch(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err = r.Create(ctx, sbomSyftFiltered("srv", "tool1"), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
			require.NoError(t, err)
			...
			case ev, ok := <-w.ResultChan():
				...
				created, ok := ev.Object.(*softwarecomposition.SBOMSyftFiltered)
				require.True(t, ok, "expected *SBOMSyftFiltered, got %T", ev.Object)
...
func TestDifferential_DryRunCreate(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sf := sbomSyftFiltered("srv", "tool1")

	var oldOut runtime.Object
	var err error
	assert.NotPanics(t, func() {
		oldOut, err = h.oldREST.Create(ctx, sf.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{DryRun: []string{metav1.DryRunAll}})
	...
	newOut, err := h.newREST.Create(ctx, sf.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{DryRun: []string{metav1.DryRunAll}})
	require.NoError(t, err)
	newMeta, err := meta.Accessor(newOut)
	require.NoError(t, err)
...
func TestDifferential_DryRunUpdate(t *testing.T) {
	...
	updatedOld := oldCreated.DeepCopy()
	updatedOld.Spec.Metadata.Tool.Name = "changed"
	var oldOut runtime.Object
	var err error
	assert.NotPanics(t, func() {
	...
	require.NoError(t, err)
	assert.Equal(t, "changed", oldOut.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)

	updatedNew := newCreated.DeepCopy()
	updatedNew.Spec.Metadata.Tool.Name = "changed"
	...
	require.NoError(t, err)
	assert.Equal(t, "changed", newOut.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name)

	// Neither implementation actually persisted the change.
	persistedOld, err := h.oldREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "tool1", persistedOld.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name, "old: dry-run update must not persist")

	persistedNew, err := h.newREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "tool1", persistedNew.(*softwarecomposition.SBOMSyftFiltered).Spec.Metadata.Tool.Name, "new: dry-run update must not persist")
}
...
func TestDifferential_ProtobufRoundTrip(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			versioned, ok := versionedOut.(*v1beta1.SBOMSyftFiltered)
			require.True(t, ok, "expected *v1beta1.SBOMSyftFiltered, got %T", versionedOut)

			data, err := versioned.Marshal()
			require.NoError(t, err)
			require.NotEmpty(t, data)

			var decoded v1beta1.SBOMSyftFiltered
