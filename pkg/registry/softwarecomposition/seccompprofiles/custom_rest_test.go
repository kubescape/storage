...
// SeccompProfile is namespaced, but its Strategy is a no-op --
// Validate/ValidateUpdate always return an empty field.ErrorList
// (strategy.go) -- so this suite has no validation-rejection cases.
...
// (checklist item 1).
func normalize(t *testing.T, obj runtime.Object) *softwarecomposition.SeccompProfile {
	t.Helper()
	sp, ok := obj.(*softwarecomposition.SeccompProfile)
	require.True(t, ok, "expected *softwarecomposition.SeccompProfile, got %T", obj)
	require.NotEmpty(t, sp.UID, "UID must be set")
	require.False(t, sp.CreationTimestamp.IsZero(), "creationTimestamp must be set")
	out := sp.DeepCopy()
	out.UID = ""
...
func seccompProfile(name, containerName string) *softwarecomposition.SeccompProfile {
	return &softwarecomposition.SeccompProfile{
		ObjectMeta: metav1.ObjectMeta{Name: name},
		Spec:       softwarecomposition.SeccompProfileSpec{Containers: []softwarecomposition.SingleSeccompProfile{{Name: containerName}}},
	}
}

func mustCreate(t *testing.T, ctx context.Context, r rest.StandardStorage, obj *softwarecomposition.SeccompProfile) runtime.Object {
	t.Helper()
...
func TestDifferential_CreateExplicitName(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sp := seccompProfile("sp-a", "container1")

	oldOut := mustCreate(t, ctx, h.oldREST, sp)
	newOut := mustCreate(t, ctx, h.newREST, sp)
	...
	_, err = h.oldREST.Create(ctx, sp.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	assert.True(t, apierrors.IsAlreadyExists(err), "old: %v", err)
	_, err = h.newREST.Create(ctx, sp.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
...
func TestDifferential_CreateGenerateName(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sp := &softwarecomposition.SeccompProfile{
		ObjectMeta: metav1.ObjectMeta{GenerateName: "sp-"},
		Spec:       softwarecomposition.SeccompProfileSpec{Containers: []softwarecomposition.SingleSeccompProfile{{Name: "container1"}}},
	}
	...
	assert.True(t, strings.HasPrefix(oldMeta.GetName(), "sp-"), "old name: %s", oldMeta.GetName())
	assert.True(t, strings.HasPrefix(newMeta.GetName(), "sp-"), "new name: %s", newMeta.GetName())
	...
	oldOut2 := mustCreate(t, ctx, h.oldREST, sp)
	newOut2 := mustCreate(t, ctx, h.newREST, sp)
...
func TestDifferential_CreateSucceeds(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sp := seccompProfile("sp-with-container", "container1")

	oldOut, err := h.oldREST.Create(ctx, sp.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	require.NoError(t, err)
	newOut, err := h.newREST.Create(ctx, sp.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	require.NoError(t, err)
	...
	oldFresh, err := h.oldREST.Get(ctx, "sp-with-container", &metav1.GetOptions{})
	require.NoError(t, err)
	newFresh, err := h.newREST.Get(ctx, "sp-with-container", &metav1.GetOptions{})
	require.NoError(t, err)
	require.Len(t, oldFresh.(*softwarecomposition.SeccompProfile).Spec.Containers, 1)
	require.Len(t, newFresh.(*softwarecomposition.SeccompProfile).Spec.Containers, 1)
	assert.Equal(t, "container1", oldFresh.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)
	assert.Equal(t, "container1", newFresh.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)
}
...
func TestDifferential_NamespaceIsolation(t *testing.T) {
	...
	mustCreate(t, ctxA, h.oldREST, seccompProfile("shared-name", "container-a"))
	mustCreate(t, ctxB, h.oldREST, seccompProfile("shared-name", "container-b"))
	mustCreate(t, ctxA, h.newREST, seccompProfile("shared-name", "container-a"))
	mustCreate(t, ctxB, h.newREST, seccompProfile("shared-name", "container-b"))

	oldA, err := h.oldREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	oldB, err := h.oldREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "container-a", oldA.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)
	assert.Equal(t, "container-b", oldB.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)

	newA, err := h.newREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	newB, err := h.newREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "container-a", newA.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)
	assert.Equal(t, "container-b", newB.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)
...
func TestDifferential_KeyDerivationMatches(t *testing.T) {
	...
	mustCreate(t, ctx, h.oldREST, seccompProfile("via-old", "container1"))
	...
	assert.Equal(t, "container1", viaNew.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)

	// Written via NEW, read via OLD.
	mustCreate(t, ctx, h.newREST, seccompProfile("via-new", "container2"))
	...
	assert.Equal(t, "container2", viaOld.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)
	...
	require.NoError(t, err)
	updated := created.(*softwarecomposition.SeccompProfile).DeepCopy()
	updated.Spec.Containers[0].Name = "container1-updated"
	...
	require.NoError(t, err)
	assert.Equal(t, "container1-updated", viaNewAfterUpdate.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)
}
...
func TestDifferential_UpdateResourceVersion(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := seccompProfile("srv", "container1")

	oldCreated := mustCreate(t, ctx, h.oldREST, base).(*softwarecomposition.SeccompProfile)
	newCreated := mustCreate(t, ctx, h.newREST, base).(*softwarecomposition.SeccompProfile)

	// Correct resourceVersion succeeds. oldCreated/newCreated come straight from
	// mustCreate's return value, which only carries ObjectMeta/SchemaVersion
	// (see saveObject/extractFields in storage.go) -- Spec.Containers is empty
	// there, so the update assigns a whole new slice rather than indexing into
	// an empty one.
	updatedOld := oldCreated.DeepCopy()
	updatedOld.Spec.Containers = []softwarecomposition.SingleSeccompProfile{{Name: "updated"}}
	...
	updatedNew.Spec.Containers = []softwarecomposition.SingleSeccompProfile{{Name: "updated"}}
...
func TestDifferential_UpdateCarriesOverProtectedFields(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base2 := seccompProfile("srv2", "container1")
	...
	require.NoError(t, err)
	oldCreated2 := oldFetched.(*softwarecomposition.SeccompProfile)
	newCreated2 := newFetched.(*softwarecomposition.SeccompProfile)
...
func TestDifferential_DeleteWithFinalizer(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := &softwarecomposition.SeccompProfile{
	...
	clearedOld := oldFresh.(*softwarecomposition.SeccompProfile).DeepCopy()
	...
	clearedNew := newFresh.(*softwarecomposition.SeccompProfile).DeepCopy()
...
func TestDifferential_DeleteCollection(t *testing.T) {
	...
	for _, name := range names {
		mustCreate(t, ctx, h.oldREST, seccompProfile(name, "container1"))
		mustCreate(t, ctx, h.newREST, seccompProfile(name, "container1"))
	}
...
func TestDifferential_Watch(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err = r.Create(ctx, seccompProfile("srv", "container1"), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
			require.NoError(t, err)
			...
			case ev, ok := <-w.ResultChan():
				...
				created, ok := ev.Object.(*softwarecomposition.SeccompProfile)
				require.True(t, ok, "expected *SeccompProfile, got %T", ev.Object)
...
func TestDifferential_DryRunCreate(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sp := seccompProfile("srv", "container1")

	var oldOut runtime.Object
	var err error
	assert.NotPanics(t, func() {
		oldOut, err = h.oldREST.Create(ctx, sp.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{DryRun: []string{metav1.DryRunAll}})
	...
	newOut, err := h.newREST.Create(ctx, sp.DeepCopy(), rest.ValidateAllObjectFunc, &metav1.CreateOptions{DryRun: []string{metav1.DryRunAll}})
	require.NoError(t, err)
	newMeta, err := meta.Accessor(newOut)
	require.NoError(t, err)
...
func TestDifferential_DryRunUpdate(t *testing.T) {
	...
	updatedOld := oldCreated.DeepCopy()
	updatedOld.Spec.Containers = []softwarecomposition.SingleSeccompProfile{{Name: "changed"}}
	var oldOut runtime.Object
	var err error
	assert.NotPanics(t, func() {
	...
	require.NoError(t, err)
	assert.Equal(t, "changed", oldOut.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)

	updatedNew := newCreated.DeepCopy()
	updatedNew.Spec.Containers = []softwarecomposition.SingleSeccompProfile{{Name: "changed"}}
	...
	require.NoError(t, err)
	assert.Equal(t, "changed", newOut.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name)

	// Neither implementation actually persisted the change.
	persistedOld, err := h.oldREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "container1", persistedOld.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name, "old: dry-run update must not persist")

	persistedNew, err := h.newREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "container1", persistedNew.(*softwarecomposition.SeccompProfile).Spec.Containers[0].Name, "new: dry-run update must not persist")
}
...
func TestDifferential_ProtobufRoundTrip(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			versioned, ok := versionedOut.(*v1beta1.SeccompProfile)
			require.True(t, ok, "expected *v1beta1.SeccompProfile, got %T", versionedOut)

			data, err := versioned.Marshal()
			require.NoError(t, err)
			require.NotEmpty(t, data)

			var decoded v1beta1.SeccompProfile
