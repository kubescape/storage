...
// SBOMSyft is namespaced, but (unlike OpenVulnerabilityExchangeContainer)
...
// (checklist item 1).
func normalize(t *testing.T, obj runtime.Object) *softwarecomposition.SBOMSyft {
	t.Helper()
	sf, ok := obj.(*softwarecomposition.SBOMSyft)
	require.True(t, ok, "expected *softwarecomposition.SBOMSyft, got %T", obj)
...
func sbomSyft(name, toolName string) *softwarecomposition.SBOMSyft {
...
func mustCreate(t *testing.T, ctx context.Context, r rest.StandardStorage, obj *softwarecomposition.SBOMSyft) runtime.Object {
	t.Helper()
...
func TestDifferential_CreateGenerateName(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sf := &softwarecomposition.SBOMSyft{
...
func TestDifferential_CreateSucceeds(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	sf := sbomSyft("sf-with-tool", "syft1")
	...
	require.NoError(t, err)
	assert.Equal(t, "syft1", oldFresh.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)
	assert.Equal(t, "syft1", newFresh.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)
}
...
func TestDifferential_NamespaceIsolation(t *testing.T) {
	...
	mustCreate(t, ctxA, h.oldREST, sbomSyft("shared-name", "tool-a"))
	mustCreate(t, ctxB, h.oldREST, sbomSyft("shared-name", "tool-b"))
	mustCreate(t, ctxA, h.newREST, sbomSyft("shared-name", "tool-a"))
	mustCreate(t, ctxB, h.newREST, sbomSyft("shared-name", "tool-b"))

	oldA, err := h.oldREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	oldB, err := h.oldREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "tool-a", oldA.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)
	assert.Equal(t, "tool-b", oldB.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)

	newA, err := h.newREST.Get(ctxA, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	newB, err := h.newREST.Get(ctxB, "shared-name", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "tool-a", newA.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)
	assert.Equal(t, "tool-b", newB.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)
...
func TestDifferential_KeyDerivationMatches(t *testing.T) {
	...
	mustCreate(t, ctx, h.oldREST, sbomSyft("via-old", "tool1"))
	...
	assert.Equal(t, "tool1", viaNew.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)

	// Written via NEW, read via OLD.
	mustCreate(t, ctx, h.newREST, sbomSyft("via-new", "tool2"))
	...
	assert.Equal(t, "tool2", viaOld.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)
	...
	require.NoError(t, err)
	updated := created.(*softwarecomposition.SBOMSyft).DeepCopy()
	...
	require.NoError(t, err)
	assert.Equal(t, "tool1-updated", viaNewAfterUpdate.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)
}
...
func TestDifferential_UpdateResourceVersion(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := sbomSyft("srv", "tool1")

	oldCreated := mustCreate(t, ctx, h.oldREST, base).(*softwarecomposition.SBOMSyft)
	newCreated := mustCreate(t, ctx, h.newREST, base).(*softwarecomposition.SBOMSyft)
...
func TestDifferential_UpdateCarriesOverProtectedFields(t *testing.T) {
	...
	require.NoError(t, err)
	oldCreated2 := oldFetched.(*softwarecomposition.SBOMSyft)
	newCreated2 := newFetched.(*softwarecomposition.SBOMSyft)
...
func TestDifferential_DeleteWithFinalizer(t *testing.T) {
	h := newHarness(t)
	ctx := testContext("ns1")
	base := &softwarecomposition.SBOMSyft{
	...
	clearedOld := oldFresh.(*softwarecomposition.SBOMSyft).DeepCopy()
	...
	clearedNew := newFresh.(*softwarecomposition.SBOMSyft).DeepCopy()
...
func TestDifferential_DeleteCollection(t *testing.T) {
	...
	for _, name := range names {
		mustCreate(t, ctx, h.oldREST, sbomSyft(name, "tool1"))
		mustCreate(t, ctx, h.newREST, sbomSyft(name, "tool1"))
	}
...
func TestDifferential_Watch(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err = r.Create(ctx, sbomSyft("srv", "tool1"), rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
			require.NoError(t, err)
			...
			case ev, ok := <-w.ResultChan():
				...
				created, ok := ev.Object.(*softwarecomposition.SBOMSyft)
				require.True(t, ok, "expected *SBOMSyft, got %T", ev.Object)
...
func TestDifferential_DryRunUpdate(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, "changed", oldOut.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)
	...
	require.NoError(t, err)
	assert.Equal(t, "changed", newOut.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name)

	// Neither implementation actually persisted the change.
	persistedOld, err := h.oldREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "tool1", persistedOld.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name, "old: dry-run update must not persist")

	persistedNew, err := h.newREST.Get(ctx, "srv", &metav1.GetOptions{})
	require.NoError(t, err)
	assert.Equal(t, "tool1", persistedNew.(*softwarecomposition.SBOMSyft).Spec.Metadata.Tool.Name, "new: dry-run update must not persist")
}
...
func TestDifferential_ProtobufRoundTrip(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			versioned, ok := versionedOut.(*v1beta1.SBOMSyft)
			require.True(t, ok, "expected *v1beta1.SBOMSyft, got %T", versionedOut)

			data, err := versioned.Marshal()
			require.NoError(t, err)
			require.NotEmpty(t, data)

			var decoded v1beta1.SBOMSyft
