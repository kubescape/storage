...
// pkg/registry/genericrest/store.go) with sbomsyfts-specific configuration,
// as a parallel, gated alternative to the genericregistry.Store based
// implementation in etcd.go (which remains the reference implementation for
// differential testing -- see custom_rest_test.go). Namespaced, no-op
// Strategy. DefaultQualifiedResource/SingularQualifiedResource are inverted
// relative to normal convention (singular/plural swapped) -- this directly
...
// sbomsyfts. Its signature intentionally matches NewREST's so it can be
...
func NewCustomREST(scheme *runtime.Scheme, storageImpl storage.Interface, optsGetter generic.RESTOptionsGetter) (*CustomREST, error) {
	...
	defaultQualifiedResource := softwarecomposition.Resource("sbomsyft")
	singularQualifiedResource := softwarecomposition.Resource("sbomsyfts")

	return genericrest.NewStore(storageImpl, optsGetter, genericrest.Store{
		NewFunc:                   func() runtime.Object { return &softwarecomposition.SBOMSyft{} },
		NewListFunc:               func() runtime.Object { return &softwarecomposition.SBOMSyftList{} },
